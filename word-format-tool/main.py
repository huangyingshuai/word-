...
def main():
    ...
    col_check, col_report = st.columns(2)
    
    with col_check:
        st.markdown("#### 在线查重")
        api_key_check = st.text_input("API Key (查重用)", type="password", key="check_api")
        check_doc_file = st.file_uploader("上传待查重文档", type=["docx", "txt"], key="check_doc")
        doc_content = ""
        if check_doc_file:
        ...
        check_text = st.text_area("待查重文本", value=doc_content if doc_content else "", height=200, key="check_text_area")
        
        if st.button("开始查重", key="start_check_btn"):
            if check_text and api_key_check:
                ...
                st.warning("请填写API Key并上传/输入文本")

    with col_report:
        st.markdown("#### 查重报告学习")
        report_file = st.file_uploader("上传查重报告 (HTML/TXT)", type=["html", "txt"], key="plag_report")
        if report_file:
            red_parts, plain_text, error = parse_plagiarism_report(report_file)
            if error:
                st.error(f"解析失败：{error}")
            elif red_parts:
                st.success(f"解析完成！发现 {len(red_parts)} 处标红内容")
                st.session_state.learned_forbidden = red_parts
                with st.expander("标红内容预览", expanded=False):
                    for i, part in enumerate(red_parts[:5]):
                        st.text(f"{i+1}. {part[:80]}...")
                
                if st.button("针对标红部分润色", key="rewrite_red_btn"):
                    with st.spinner("正在润色..."):
                        new_text = plain_text
                        change_count = 0
                        for part in red_parts:
                            if len(part) > 10:
                                modified, _ = rewrite_paragraph(part, REWRITE_LEVEL["标准润色"], api_key, red_parts)
                                new_text = new_text.replace(part, modified)
                                change_count += 1
                        st.text_area("润色后文本", new_text, height=200)
                        st.success(f"润色完成！共优化 {change_count} 处")
